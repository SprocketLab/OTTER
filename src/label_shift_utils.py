...
def get_uniformly_resampled_indices(y, num_labels, n_samples_per_class, seed):
    np.random.seed(seed)
    # get indices for each label
    indices_by_label = [(y == k).nonzero()[0] for k in range(num_labels)]
    num_samples = n_samples_per_class

    agg_idx = []
    for i in range(num_labels):
        # sample an example from X with replacement
        if num_samples > len(indices_by_label[i]):
            idx = np.random.choice(
                indices_by_label[i], size=num_samples, replace=True
            )
        else:
            idx = np.random.choice(
                indices_by_label[i], size=num_samples, replace=False
            )
        agg_idx.append(idx)

    return np.concatenate(agg_idx)

def get_uniformly_resampled_indices_split(y, num_labels, n_samples_per_class, seed, train_split_ratio=0.5):
    np.random.seed(seed)
    # get indices for each label
    indices_by_label = [(y == k).nonzero()[0] for k in range(num_labels)]
    num_samples = n_samples_per_class
    num_train_samples = int(num_samples * train_split_ratio)
    num_val_samples = int(num_samples * (1-train_split_ratio))
    ...
    for i in range(num_labels):
        ...
        val_idx.append(idx[num_train_samples:])

    return np.concatenate(train_idx), np.concatenate(val_idx), np.concatenate(train_idx + val_idx)
...
def get_resampled_indices(y, num_labels, Py, seed):
    ...
    THRESHOLD = 1e-8
    num_samples = int(
        min([len(indices_by_label[i]) / Py[i] for i in range(num_labels) if Py[i] > THRESHOLD])
    )
...
#     return np.multiply(wt_soft, py_true_source)
    py_target = np.multiply(wt_hard, py_true_source)
    py_target /= py_target.sum()
    return py_target
...
#     return np.multiply(wt_soft,  mu_train_y)
    
    return np.multiply(wt_hard, mu_train_y)
